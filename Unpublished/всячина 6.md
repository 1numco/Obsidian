...
- это происходит, потому что операции над volatile переменными становятся видимыми сайд-эффектами. Такие операции в программе влияют на другие потоки и внешние системы. Компилятор в принципе может крутить ваш код на всех прологоватых инструментах, которых он хочет. Главное, чтобы видимое внешнему миру исполнение осталось прежним. Поэтому просто выкинуть из кода использование volatile переменной он не может.
...
- операции над такими переменными не являются гарантированно атомарными в том плане, что есть возможность увидеть их промежуточное состояние. В целом, ничто не мешает пометить volatile объект std::unordered_map и конечно же операции над мапой не будут атомарными. Они могут быть атомарными на определенных архитектурах с правильным выравниванием памяти, но это никто не гарантирует.
...
- операции над атомиками - это также видимые спецэффекты, поэтому компилятор их не может выкинуть из кода.
...
Начнем с const. Наш старый добрый друг с первых дней C++ (а также C), может быть применен к объектам, чтобы указать на их неизменяемость. Попытка изменить const переменную напрямую приведет к ошибке компиляции, а через грязные хаки - к UB.
...
И ровно для этой цели можно помечать переменную constexpr. Вы так приказываете компилятору вычислить переменную в compile-time. Главное, чтобы инициализатор также был constant expression:
...
consteval может быть применен только к функцим и это ключевое слово заставляет функцию выполняться во время компиляции и возвращать константное выражение. Отсюда и название "константные вычисления".
...
Интересность ситуёвины состоит в том, что constinit не подразумевает константность объекта! Действительно, мы же можем задать неконстантной переменной какое-то определенное начальное значение и это будет валидный код:
...
С появлением захвата this по значению стала очень путающей семантика неявного захвата. Что reference capture &, что value capture =, захватывали this по ссылке. И ничто неявно не захватывало this по значению.
...
Захват this переживает объект, решаем с помощью std::enable_shared_from_this
